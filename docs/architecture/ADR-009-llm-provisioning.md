...
**단일 출처 하나에서 pi 의 `models.json` 을 생성한다.** 그 출처는 처음에 `.env`
였고, **2026-09-03 에 운영 DB 로 옮겼다** (아래 §개정).
...
### 개정 (2026-09-03) — 단일 출처를 운영 DB 로 옮긴다  {#rev-db}

`.env` 는 **씨앗**이 되고 SSOT 는 운영 DB 의 `llm_endpoint` 한 행이다. 국면(phase)이
이미 같은 형태다 — 설정은 처음 한 번을 정하고, 그 뒤로 값을 바꾸는 것은 시스템 안에서
일어난다.

**옮긴 이유는 하나다.** 연결을 바꾸는 길이 파일 편집과 두 프로세스 재기동뿐이면
**대시보드에서 할 수 없는 일**이 되고, §8.1 에 따르면 그것은 곧 아무도 할 수 없는
일이다 (FR-62). 운영자는 겸업 1인이고 모델이 붙었는지조차 화면에서 보이지 않았다.

| | 어디 | 언제 읽히는가 |
|---|---|---|
| 씨앗 | `.env` (`ASD_LLM_*`) | DB 에 행이 없을 때 **한 번** |
| SSOT | `llm_endpoint` (운영 DB) | 웹은 요청마다 · 워커는 틱마다 |
| pi | `~/.pi/agent/models.json` | **저장하는 그 자리에서 다시 쓴다** |

**검문소는 옮기지 않았다.** 저장이 `assert_endpoint_allowed` 를 지나며, 통과하지
못하면 DB 도 `models.json` 도 바뀌지 않는다 — 화면이 생겼다고 NFR-1 을 우회하는
문이 열려서는 안 된다. 오히려 검사가 한 번 더 는다: 저장할 때와 `models.json` 을
쓸 때다.

**키는 여전히 `.env` 에만 있다.** 화면은 키를 받지 않고 **있고 없음만** 말한다.
DB 에 넣으면 `var/` 의 파일 하나가 자격증명을 들게 되고, `models.json` 이 `$VAR`
참조만 담기로 한 이 ADR 의 결정과도 어긋난다.
...
- (2026-09-03) 주소·모델·출력 한도의 SSOT 는 **운영 DB** 이고 `.env` 는 씨앗이다 — 화면에서 바꾸면 재기동 없이 다음 생성에 쓰인다
