...
- 배포 환경: **http 만 가능하고, 지금 쓰는 곳은 개발 전용이다** (2026-09-02 확인). 그래서 대시보드 세션 쿠키에 `secure` 를 켜지 않는다 — 켜면 쿠키가 저장되지 않아 **인증을 켠 순간 로그인할 수 없게 된다.** 평문 전송의 도청 위험은 **개발 전용인 동안 문제가 아니다.** 실제 모 시스템을 붙이는 날 다시 볼 판단이고, 그 전환은 기동 게이트가 이미 잡는다 (WBS-5.2.1)
