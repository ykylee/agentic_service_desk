...
def set_cookie(response, token: str, *, ttl_hours: int) -> None:  # noqa: ANN001
    ...
    `secure` 는 켜지 않는다. **사내 환경이 http 만 가능하다**(2026-09-02 확인,
    `docs/PROJECT_PROFILE.md` §5) — 임시 타협이 아니라 확정 제약이다. 켜면 그
    환경에서 쿠키가 아예 저장되지 않아 **인증을 켠 순간 로그인할 수 없게 된다.**
    설정으로 빼지 않은 것도 같은 이유다: 켤 수 있는 환경이 오지 않는데 값만
    늘리면, 언젠가 누가 켜서 로그인을 막는 길만 남는다.

    **대신 무엇이 흐르는지는 분명히 해 둔다.** 평문이므로 같은 구간을 엿듣는
    쪽은 로그인 때 암호를, 이후에는 이 쿠키를 본다. 쿠키가 암호 자체가 아닌
    HMAC 인 것이 여기서 값을 한다 — 훔친 쪽은 **만료까지 그 세션을 재생할 수
    있지만 암호를 얻지는 못한다.** 남는 방어는 전송이 아니라 **닿을 수 있는
    범위**이고, 그것이 `web_host` 를 좁게 두라는 규칙과 인증 없이 루프백 밖으로
    나가면 기동을 거부하는 규칙(WBS-5.2.1)이 서 있는 이유다.
    """
    response.set_cookie(
