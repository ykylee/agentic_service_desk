...
def set_cookie(response, token: str, *, ttl_hours: int) -> None:  # noqa: ANN001
    ...
    `secure` 는 켜지 않는다. **http 만 가능한 환경이고 지금 쓰는 곳은 개발
    전용이다**(2026-09-02 확인, `docs/PROJECT_PROFILE.md` §5). 켜면 쿠키가 아예
    저장되지 않아 **인증을 켠 순간 로그인할 수 없게 된다.** 설정으로 빼지 않은
    것도 같은 이유다: 켤 수 있는 환경이 아닌데 값만 늘리면, 언젠가 누가 켜서
    로그인을 막는 길만 남는다.

    **평문이라는 사실 자체는 남는다** — 같은 구간을 엿듣는 쪽은 로그인 때 암호를,
    이후에는 이 쿠키를 본다. 쿠키가 암호 자체가 아닌 HMAC 인 것이 그때 값을
    한다(훔친 쪽은 만료까지 재생할 수 있지만 암호를 얻지는 못한다). 다만
    **개발 전용인 동안 그것은 위험이 아니다.** 다시 볼 조건은 실제 모 시스템을
    붙이는 날 하나이고, 그 전환은 `preflight.check_live_exposure` 가 잡는다.
    """
    response.set_cookie(
