...
class Settings(BaseSettings):
    ...
    llm_max_output_tokens: int = Field(
        default=32_768,
        description=(
            "한 번의 생성에서 모델이 낼 수 있는 최대 토큰. **pi 의 `models.json` 에 "
            "`maxTokens` 로 실린다.**\n\n"
            "지정하지 않으면 pi 기본값 **16,384** 가 쓰이는데, 그 값이 2026-08~09 "
            "부트스트랩 실패의 **59/62** 를 만들었다. 사고를 길게 하는 모델이라 "
            "예산을 사고에 다 쓰고 `stop=length` 로 잘렸고, 그러면 두 모양으로 "
            "나타난다 — 사고가 예산을 **전부** 먹으면 `strip_thinking()` 뒤에 "
            "**본문이 빈 문자열**이 되고(빈 응답 48건), **거의 다** 먹으면 JSON 을 "
            "쓰다 잘린다(파싱 실패 11건).\n\n"
            "실측(2026-09-02): 같은 묶음이 16,384 에서는 본문 0자였는데 32,768 에서는 "
            "`stop=stop` 으로 끝나며 본문이 나왔다 — 그 호출들이 실제로 쓴 것은 "
            "18,184 · 23,953 토큰이고 그중 사고가 16,806 · 22,553 이었다. "
            "**한도를 올린 것이 아니라, 정하지 않던 것을 정한 것이다.**"
        ),
    )
    web_host: str = Field(
        default="127.0.0.1",
